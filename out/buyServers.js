    ...
    if (pickedRam.bool) {
        ...
        for (let i of j) {
            if (i === 's-0') {
                ...
                ns.clearPort(5);
                await ns.writePort(5, 'true'); // startup에서 darkweb 실행 조건을 위해 넣은것!
                break;
            }
        }
        ...
        if (isSmushed === 'true') {
            ...
            if (pickedRam.ram < jServerRam) {
                ns.tprint(`ERROR 💻 돈이 적음! 돈 생길때까지 루프 검사 할꺼임`);
                while (pickedRam.ram < jServerRam) {
                    pickedRam = selectServerRam(ns);
                    await ns.sleep(20000);
                    ns.tprint(`WARN 💻 ${jServerRam} GB 될 때까지 돈 기둘리는 중... 현재 ${pickedRam.ram} GB `);
                }
        ...
        if (doIhaveServers && pickedRam.ram <= jServerRam) {
            ns.tprint(`서버 냅둠 / 현재 서버: ${jServerRam} GB / 지금 고른 램: ${pickedRam.ram} GB`);
            return;
        }

        // 이전 램 보다 클 때
        if (pickedRam.ram > jServerRam) {
            ns.tprint(`WARN 💻 서버 업글: ${jServerRam} GB -> ${pickedRam.ram} GB`);
            await installServer(ns, pickedRam);
            return;
        }
    } else
