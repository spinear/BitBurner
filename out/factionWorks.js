...
function selectFaction(_ns) {
    ...
    fwo.augList.forEach((augList, i) => {
        ownedAugs.some(ownedAug => {
            if (ownedAug === augList || ns.getFactionRep(fwo.factionList[i]) > fwo.repCost[i]) {
                ns.tprint(`INFO ${augList}은(는) 이미 먹었거나 REP(${fwo.repCost[i]})이 충분해 ${fwo.factionList[i]} 팩션은 재낌`);
